                       ...
                       <tbody>
                            @foreach($pelanggaran as $p)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge badge-pill badge-outline-{{ $p->kategori == 'berat' ? 'danger' : ($p->kategori == 'sedang' ? 'warning' : 'info') }}">
                                            {{ ucfirst($p->kategori) }}
                                        </span>
                                    </td>
                                    <td>{{ $p->poin }}</td>
                                    <td>
                                        <span class="badge badge-pill badge-{{ $p->status == 'selesai' ? 'success' : 'secondary' }}">
                                            {{ ucfirst($p->status) }}
                                        </span>
                                    </td>
                                    <td>
                                        @can('process-violations')
                                            <a href="{{ route('pelanggaran-siswa.edit', $p->id) }}" class="btn btn-xs btn-outline-primary">
