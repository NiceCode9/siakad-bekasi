    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="card mb-4">
                    <div class="card-body">
                        <h5 class="card-title mb-4">Buat Legger Baru</h5>
                            ...
                            <div class="form-group">
                                ...
                                <select name="semester_id" class="form-control select2-single" required>
                                    ...
                                    @foreach ($semesters as $semester)
                                        <option value="{{ $semester->id }}">{{ $semester->tahunAkademik->nama }} -
                                            {{ $semester->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <button type="submit" class="btn btn-primary">BUAT LEGGER</button>
                        </form>
                    </div>
                </div>
            </div>
                            ...
                            <tbody>
                                @foreach ($leggers as $l)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('legger.show', $l->id) }}"
                                                class="btn btn-xs btn-outline-primary">Lihat</a>
                                            <a href="{{ route('legger.excel', $l->id) }}"
                                                class="btn btn-xs btn-outline-success">Excel</a>
                                            <a href="{{ route('legger.pdf', $l->id) }}"
                                                class="btn btn-xs btn-outline-danger">PDF</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
