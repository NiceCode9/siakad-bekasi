                    ...
                    <div class="card-body">
                        ...
                        @if($soal->tipe_media)
                            <div class="alert alert-light border py-3 mb-4">
                                <div class="d-flex align-items-center mb-2">
                                    <span class="badge badge-info mr-2">{{ ucfirst($soal->tipe_media) }}</span>
                                    <span class="small text-muted font-weight-bold text-uppercase">Media Saat Ini</span>
                                </div>
                                <div class="media-preview bg-white p-2 rounded border text-center">
                                    @if($soal->tipe_media == 'image')
                                        <img src="{{ asset('storage/'.$soal->file) }}" class="img-fluid rounded" style="max-height: 200px;">
                                    @elseif($soal->tipe_media == 'audio')
                                        <audio controls src="{{ asset('storage/'.$soal->file) }}" class="w-100"></audio>
                                    @elseif($soal->tipe_media == 'video')
                                        <video controls src="{{ asset('storage/'.$soal->file) }}" class="w-100 rounded" style="max-height: 300px;"></video>
                                    @endif
                                </div>
                                <div class="mt-2">
                                    <small class="text-muted"><i class="fas fa-info-circle mr-1"></i> Upload file baru di bawah jika Anda ingin mengganti media saat ini.</small>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="dropzone" id="mediaDropzone">
                            <div class="dz-message">
                                <i class="simple-icon-cloud-upload d-block h3 mb-2"></i>
                                <span>Tarik file ke sini untuk mengganti/menambah media</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm mb-4" id="containerOptions">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <span class="badge badge-primary badge-pill px-3 py-2" id="typeLabel">{{ ucfirst(str_replace('_', ' ', $soal->tipe_soal)) }}</span>
                        </div>
                        ...
                        <div class="mt-4">
                                        ...
                                        <div class="input-group-prepend">
                                            <div class="input-group-text bg-light border-right-0">
                                                <div class="custom-control custom-radio">
                                                    <input type="radio" id="check_{{ $k }}" name="kunci_jawaban" value="{{ strtoupper($k) }}" class="custom-control-input" {{ $soal->kunci_jawaban == strtoupper($k) ? 'checked' : '' }}>
                                                    <label class="custom-control-label font-weight-bold" for="check_{{ $k }}">{{ strtoupper($k) }}</label>
                                                </div>
                                            </div>
                                        </div>
                                        <input type="text" name="opsi_{{ $k }}" class="form-control border-left-0 pl-0" value="{{ $soal->$field }}" placeholder="Ketikkan teks jawaban untuk opsi {{ strtoupper($k) }}...">
                                    </div>
                                @endforeach
                            ...
                            <div id="blockIsian" style="{{ $soal->tipe_soal != 'isian_singkat' ? 'display:none;' : '' }}">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="kunci_jawaban_text" class="form-control form-control-lg" value="{{ $soal->kunci_jawaban }}" placeholder="Masukkan jawaban singkat yang benar...">
            ...
            <div class="col-md-3">
                <div class="card border-0 shadow-sm mb-4 sticky-top" style="top: 100px; z-index: 10;">
                    <div class="card-body">
                        ...
                        <a href="{{ route('bank-soal.show', $soal->bank_soal_id) }}" class="btn btn-outline-secondary btn-block mt-2">
                            Batal
                        </a>
                    </div>
                </div>
    ...
    $(document).ready(function() {
        ...
        const mediaDropzone = new Dropzone("#mediaDropzone", {
            ...
            init: function() {
                ...
                this.on("error", function(file, response) {
                    $('#btnSubmit').prop('disabled', false).html('<i class="fas fa-save mr-1"></i> Simpan Perubahan');
                    let errorMessage = typeof response === 'string' ? response : (response.message || 'Gagal update soal.');
        ...
        function submitFormNormally() {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    $('#btnSubmit').prop('disabled', false).html('<i class="fas fa-save mr-1"></i> Simpan Perubahan');
                    let errorMessage = xhr.responseJSON.message || 'Gagal update soal.';
